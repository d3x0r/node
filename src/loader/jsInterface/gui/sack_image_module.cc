...
void ColorObject::toString( const FunctionCallbackInfo<Value>& args ) {
	...
	char buf[128];
	int len = snprintf( buf, 128, "{r:%d,g:%d,b:%d,a:%d}", RedVal( co->color ), GreenVal( co->color ), BlueVal( co->color ), AlphaVal( co->color ) );
	
	args.GetReturnValue().Set(localStringExternal(isolate, buf, len) );
}
	...
	if( args.IsConstructCall() ) {
		...
		if( argc == 1 ) {
			if( args[0]->IsObject() ) {
				...
				r = (int)o->Get( context, localStringExternal( isolate, "r", 1 ) ).ToLocalChecked()->NumberValue(context).ToChecked();
				grn = (int)o->Get( context, localStringExternal( isolate, "g", 1 ) ).ToLocalChecked()->NumberValue(context).ToChecked();
				b = (int)o->Get( context, localStringExternal( isolate, "b", 1 ) ).ToLocalChecked()->NumberValue(context).ToChecked();
				a = (int)o->Get( context, localStringExternal( isolate, "a", 1 ) ).ToLocalChecked()-> NumberValue(context).ToChecked();
